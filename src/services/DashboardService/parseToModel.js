...
import { convertToDataLabel, convertTitleCase, intArrayStat } from './util';
...
export const convertDashboardModel = (response, chart, detailCharts, keys = ['filters', 'result_filtered'], time) =>
{
    ...
    if (buckets && Array.isArray(buckets))
    {
        ...
        const items = buckets.map((item, idx) =>
        {
            if (detailCharts)
            {
                ...
                detailCharts.forEach((element, indexDetail) =>
                {
                    ...
                    {
                        const data = intArrayStat(element?.initValue, element?.timeRange, time);
                        if (data)
                        {
                    ...
                    _.set(dataDetailChart, [indexDetail, 'types', idx], { color: chart.colors[idx], typeName: item.val });
                    _.set(dataDetailChart, [indexDetail, 'title'], element?.label);
                    _.set(dataDetailChart, [indexDetail, 'type'], element?.key);

                    if (element?.detailType === 'function' && childBuckets)
                    {
                        _.unset(childBuckets, 'count');
                        _.unset(childBuckets, 'val');
                        childBuckets = Object.entries(childBuckets).map(([key, val]) => ({ 'val': key.replace('val$', ''), count: val.count }));
                    }

                    childBuckets && _.map(childBuckets, child =>
                    {
                        const val = convertToDataLabel(child.val, element?.timeRange, element?.detailType === 'function');
                        ...
                        _.set(dataDetailChart, [indexDetail, 'values', indexLabel, idx], valuePrev + child.count);
                        _.set(dataDetailChart, [indexDetail, 'labels'], labels);
                    });
                });
            }

            const percent = Math.floor((item.count / total) * 100);
        ...
        _.forEach(chart?.overviewRatio?.variables, (item, index) =>
        {
            ...
            if (overview_number)
            {
                pairValue[`number${index + 1}`] = Object.values(overview_number)[1] ?? Object.values(overview_number)[0];
            }
        });

        let ratioValue = 'N/A';
        if (chart && chart.overviewRatio)
        {
            ratioValue = math_exp.eval(chart?.overviewRatio?.evaluator, varTokens, pairValue);
        }

...
export class DirectionService
{
    ...
    searchAll = (text, bounds) => this.client.get('/api/locations/search', {
        keyword: text,
        lx: bounds.west,
        ly: bounds.north,
        rx: bounds.east,
    ...
    {
        return this.client.get(`/api/viaroute?locs=${CommonHelper.coordsEncode(locs, 6)}&veh=${veh}&crit=${crit}&geom=1&steps=${step}&user=${user}&barr=${isAvoidBarrier ? 1 : 0}`).then((res) =>
        {
            if (res.status !== 200)
            {
                return null;
    ...
    getRouteAvoidBarrierDebounced = AwesomeDebouncePromise(this.getRouteAvoidBarrier, 200);

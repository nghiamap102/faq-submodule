...
const prepare = (doc: Document): void =>
{
    doc.addEventListener('keydown', handleKeyDown, true);
    doc.addEventListener('mousedown', handlePointerDown, true);
    doc.addEventListener('pointerdown', handlePointerDown, true);
    doc.addEventListener('touchstart', handlePointerDown, true);
    doc.addEventListener('visibilitychange', () => handleVisibilityChange(doc), true);
};

export const teardown = (doc: Document): void =>
{
    doc.removeEventListener('keydown', handleKeyDown, true);
    doc.removeEventListener('mousedown', handlePointerDown, true);
    doc.removeEventListener('pointerdown', handlePointerDown, true);
    doc.removeEventListener('touchstart', handlePointerDown, true);
    doc.removeEventListener('visibilitychange', () => handleVisibilityChange(doc), true);
};

const isFocusVisible = (event: React.FocusEvent): boolean =>
{
    const { target } = event;
    try
    {
...
export const useIsFocusVisible = <E extends HTMLElement = HTMLElement>(): UseIsFocusVisibleResult<E> =>
{
   ...
   * Should be called if a blur event is fired
   */
    const handleFocusVisible = (event: React.FocusEvent) =>
    {

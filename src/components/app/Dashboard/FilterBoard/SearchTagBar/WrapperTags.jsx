...
import { convertCustomTime, getCurrentMainTab, rebuildOptionData } from 'services/DashboardService/util';

const WrapperTags = ({ dashboardStore, children, className, isShowAll, isBrief }) =>
{
    ...
    const getTagsByType = (item) =>
    {
        ...
        else
        {
            if (isBrief)
            {
                ...
                {
                    const tag = _.find(item?.data, { id: sel });
                    return tag.label;
                }), ', ');
    ...
    const getTagPeriod = (item) =>
    {
        ...
        return [{
            id: item?.key,
            name: value.type === 'Custom' ? `${fromDate.format('MMM D, YYYY')} - ${toDate.isSame(moment(), 'd') ? t('Hôm nay') : toDate.format('MMM D, YYYY')} ` : tag.label,
        }];
    };
...
export default inject('dashboardStore')(observer(WrapperTags));

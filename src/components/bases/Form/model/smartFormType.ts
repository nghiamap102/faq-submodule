...
export type DefaultErrorMsgKey = keyof Pick<RegisterOptions, 'required' | 'min' | 'max' | 'minLength' | 'maxLength' | 'pattern' | 'validate'>;

export type FormStateCollection = Record<string, FormState<FieldValues>>
...
export type JsonFormDynamicField<Components extends DynamicFormComponents = DynamicFormComponents> = {
    [Component in Components]: {
        component: Component
        props: MappedFormComponentProps[Component]
    } & {holder: Component extends FormFieldComponents ? {fields?: never} : {fields: JsonFormDynamicField[] }}['holder']
}[Components]

export type DynamicFormComponents = FormFieldComponents | WrapperComponents
export type FormFieldComponents = 'Input' | 'FormControlLabel' | 'EmptyButton' | 'Button' | 'CheckBox' | 'RichText' | 'AdvanceSelect' | 'DateTimePicker' | 'MultilineInput'
export type WrapperComponents = 'FieldSet' | 'Container' | 'Row' | 'Column' | 'Row2' | 'Col2' | 'InputGroup' | 'InputAppend' | 'InputPrepend'

type MappedFormComponentProps = {
    Input: InputProps
    ...
    // Todo: Enhance below components by typescript
